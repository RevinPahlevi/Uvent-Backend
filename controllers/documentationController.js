...
// Fungsi untuk mengupdate dokumentasi
exports.updateDocumentation = async (req, res) => {
    try {
        const { id } = req.params;
        const { user_id, description, photo_uri } = req.body;

        console.log("=== UPDATE DOCUMENTATION DEBUG ===");
        console.log("Documentation ID:", id);
        console.log("User ID:", user_id);

        // Cek apakah dokumentasi ada dan milik user yang sama
        const [doc] = await db.query(
            'SELECT user_id FROM documentations WHERE id = ?',
            [id]
        );
        ...
        if (doc[0].user_id !== parseInt(user_id)) {
            return res.status(403).json({ status: 'fail', message: 'Anda tidak memiliki izin untuk mengubah dokumentasi ini' });
        }

        const sql = `UPDATE documentations SET 
                        description = COALESCE(?, description),
                        photo_uri = ?
                     WHERE id = ?`;

        await db.query(sql, [description, photo_uri || null, id]);

        res.status(200).json({ status: 'success', message: 'Dokumentasi berhasil diperbarui' });
    } catch (error) {
